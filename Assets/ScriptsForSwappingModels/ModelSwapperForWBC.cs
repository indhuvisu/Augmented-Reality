...
public class ModelSwapperForWBC : MonoBehaviour, ITrackableEventHandler 
{
	...
	private void SwapToWithParts() {
		...
		if (count == 0) {
			...
			WithPartPrefab.transform.localPosition=new Vector3(0f,0.479f,-0.005f);
			WithPartPrefab.transform.localRotation=Quaternion.Euler(0f,0f,0f);
			WithPartPrefab.transform.localScale=new Vector3(0.01f,0.01f,0.01f);
